...
def LogisticRegression():
    ...
    print('X.shape',X.shape)   #118行，6列
    initial_theta = np.zeros((X.shape[1],1))#初始化theta

    print('initial_theta:',initial_theta)  #6行1列
...
def mapFeature(X1,X2):
    ...
    '''
    这里以degree=2为例，映射为x1,x2,x1^2,x1*x2,x2^2
    '''
    for i in np.arange(1,degree+1): 
        for j in range(i+1):
            temp = X1**(i-j)*(X2**j)    #矩阵直接乘相当于matlab中的点乘.*
            #print('temp:',temp)
            #print('temp.reshape',temp.reshape(-1,1))
...
def costFunction(initial_theta,X,y,inital_lambda):
    ...
    h = sigmoid(np.dot(X,initial_theta))    # 计算h(z)   x与theta相乘，得Z
...
def gradient(initial_theta,X,y,inital_lambda):
    ...
    print('theta1',theta1)
    theta1[0] = 0    #因为正则化的梯度从j=1开始，所以，虽然下轮迭代时用上次更新的值，但是theta1[0]都要置0.
